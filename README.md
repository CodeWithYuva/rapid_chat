...
✅ Built for speed: ⏱️ <15s response time on GPU  and average of 50s and <100s time in cpu
✅ Completely **offline** 
...
MAX_TOKENS	100	Max token length per chunk (optimizes speed + context)
MIN_TOKENS	60	Min token length to avoid fragmentation
...
Check token length of each paragraph

If paragraph has < MIN_TOKENS (60):

We merge it with the next paragraph, to form a more complete context.

If paragraph has > MAX_TOKENS (100):

We apply sliding window token chunking:
...
If merging short paragraphs exceeds 100 tokens → we break it back into sub-chunks.

If a document has long lists or tables, we chunk them based on token count alone.
